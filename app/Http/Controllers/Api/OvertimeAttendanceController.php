...
class OvertimeAttendanceController extends Controller
{
    ...
    public function checkIn(Request $request)
    {
        ...
        $now = now('UTC');
        $normal = $this->normalAttendance($employee->company_id, $employee->id, $businessNow);
        ...
        $assignment = $this->proofs->assignment($employee, $businessNow);
        ...
        $existing = OvertimeAttendance::query()->where('company_id', $employee->company_id)->where('employee_id', $employee->id)->whereDate('date', $businessNow)->first();
        if ($existing?->clock_in_at) throw ValidationException::withMessages(['overtime' => ['Presensi masuk lembur hari ini sudah tercatat.']]);
        $distance = $this->proofs->assertGeofence($data, [...$policy, 'geofence_required' => false]);
        $overtime = $existing ?: new OvertimeAttendance();
        $overtime->fill([
            'company_id' => $employee->company_id, 'employee_id' => $employee->id, 'attendance_id' => $normal->id,
            'attendance_shift_id' => $assignment?->attendance_shift_id, 'date' => $businessNow->toDateString(), 'clock_in_at' => $now,
            'in_latitude' => $data['latitude'], 'in_longitude' => $data['longitude'], 'gps_accuracy_meters' => $data['accuracy'] ?? null,
            'geofence_distance_meters' => $distance, 'geofence_validated' => false, 'in_photo' => $this->storeProof($request, $employee->company_id, $employee->id, $businessNow, 'in'),
        ...
        return response()->json(['data' => $this->payload($overtime->fresh(), $policy['max_minutes'], $timezone), 'message' => 'Presensi masuk lembur berhasil dicatat.'], 201);
    }

    public function checkOut(Request $request)
    {
        $employee = $this->proofs->employeeFor($request->user()); $timezone = $this->timezoneFor($employee); $businessNow = now($timezone); $now = now('UTC');
        $assignment = $this->proofs->assignment($employee, $businessNow); $policy = $this->policy($employee, $assignment);
        $overtime = OvertimeAttendance::query()->where('company_id', $employee->company_id)->where('employee_id', $employee->id)->whereDate('date', $businessNow)->first();
        ...
        $data = $this->validatedProof($request, $policy['selfie_required']); $distance = $this->proofs->assertGeofence($data, [...$policy, 'geofence_required' => false]);
        ...
        $overtime->update(['clock_out_at' => $countedOut, 'out_latitude' => $data['latitude'], 'out_longitude' => $data['longitude'],
            'gps_accuracy_meters' => $data['accuracy'] ?? $overtime->gps_accuracy_meters, 'geofence_distance_meters' => $distance, 'geofence_validated' => false,
            'out_photo' => $this->storeProof($request, $employee->company_id, $employee->id, $businessNow, 'out'),
        ...
        return response()->json(['data' => $this->payload($overtime->fresh(), $policy['max_minutes'], $timezone), 'message' => 'Presensi keluar lembur berhasil dicatat.']);
    }

    public function today(Request $request)
    {
        $employee = $this->proofs->employeeFor($request->user()); $timezone = $this->timezoneFor($employee); $businessNow = now($timezone); $assignment = $this->proofs->assignment($employee, $businessNow); $policy = $this->policy($employee, $assignment);
        $item = OvertimeAttendance::query()->where('company_id', $employee->company_id)->where('employee_id', $employee->id)->whereDate('date', $businessNow)->first();
        return response()->json(['data' => $this->payload($item, $policy['max_minutes'], $timezone)]);
    }
    ...
    public function history(Request $request)
    {
        ...
        $items = OvertimeAttendance::query()->where('company_id', $employee->company_id)->where('employee_id', $employee->id)->latest('date')->limit(90)->get()
            ->map(fn (OvertimeAttendance $item) => $this->payload($item, 180, $timezone));
    ...
    private function payload(?OvertimeAttendance $item, int $maxMinutes, string $timezone): array { $minutes = $item?->clock_in_at ? max(0, $item->clock_in_at->diffInMinutes($item->clock_out_at ?: now('UTC'))) : 0; return ['id' => $item?->id, 'clock_in' => $item?->clock_in_at?->copy()->setTimezone($timezone)->format('H:i'), 'clock_out' => $item?->clock_out_at?->copy()->setTimezone($timezone)->format('H:i'), 'clock_in_at' => $item?->clock_in_at?->toIso8601String(), 'clock_out_at' => $item?->clock_out_at?->toIso8601String(), 'work_minutes' => $minutes, 'work_hours' => sprintf('%02d:%02d', intdiv($minutes,60),$minutes%60), 'max_minutes' => $maxMinutes, 'in_selfie_url' => $item?->in_photo ? url("/api/v1/overtime/history/{$item->id}/proof/in") : null, 'out_selfie_url' => $item?->out_photo ? url("/api/v1/overtime/history/{$item->id}/proof/out") : null]; }
    private function timezoneFor($employee): string { $timezone = (string) ($employee->company?->timezone ?: 'Asia/Jakarta'); return in_array($timezone, timezone_identifiers_list(), true) ? $timezone : 'Asia/Jakarta'; }
}
