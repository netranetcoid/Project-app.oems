...
<style>
  ...
  .tracking-motor { width: 42px; height: 42px; display: grid; place-items: center; border-radius: 50%; background: #0f4c81; border: 3px solid #fff; box-shadow: 0 5px 14px rgba(15, 23, 42, .35); font-size: 21px; }
...
<div class="card mb-4">
  <div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-2"><div><h5 class="mb-0">Rute perjalanan</h5><small class="text-muted">Biru = rute valid, kuning = perlu review, merah = fake GPS. Ikon pegawai-motor menunjukkan titik terakhir.</small></div><span class="badge bg-label-primary">{{ $date }}</span></div>
  <div class="card-body"><div class="row g-3"><div class="col-lg-8"><div id="workTrackingMap" style="height:560px" class="rounded"></div></div><div class="col-lg-4"><div id="routeDetail" class="tracking-detail"><h5 class="mb-2">Detail perjalanan</h5><p class="text-muted mb-0">Klik titik, ikon pegawai-motor, garis rute, atau baris pegawai untuk melihat detailnya.</p></div></div></div></div>
</div>
...
<script>
  ...
  const showDetail = (route, selectedPoint = null) => { const last=route.at(-1), point=selectedPoint || last, stop=stopInfo(route), isLastPoint=point === last, stopped=isLastPoint && stop.seconds>=600; detail.innerHTML=`<div class="d-flex align-items-center gap-2"><span class="tracking-pulse"></span><div><h5 class="mb-0">${point.name || 'Pegawai'}</h5><small class="text-muted">${point.email || '-'}</small></div></div><div class="tracking-stat"><small class="text-muted d-block">${isLastPoint ? 'Titik terakhir' : 'Titik riwayat'}</small><strong>${point.lat.toFixed(6)}, ${point.lng.toFixed(6)}</strong><small class="text-muted d-block mt-1">Terekam ${point.time} - ${point.mode === 'overtime' ? 'Lembur' : 'Jam kerja'}</small></div><div class="tracking-stat"><small class="text-muted d-block">Status lokasi</small><strong>${isLastPoint ? (stopped ? 'Berhenti sekitar '+duration(stop.seconds) : 'Bergerak / belum 10 menit') : 'Titik perjalanan'}</strong><small class="text-muted d-block mt-1">Status berhenti dihitung pada titik terakhir dalam radius 35 m.</small></div><div class="tracking-stat"><small class="text-muted d-block">Validasi sistem</small><strong class="${point.status==='blocked'?'text-danger':point.status==='review'?'text-warning':'text-success'}">${statusLabel(point.status)}</strong></div>`; };
  routes.forEach((route)=>{ const valid=route.filter((point)=>point.status==='accepted'); if(valid.length>1){const line=L.polyline(valid.map((point)=>[point.lat,point.lng]),{color:'#0284c7',weight:5,opacity:.85}).addTo(map);line.on('click',()=>showDetail(route));} route.forEach((point,pointIndex)=>{const color=colorFor(point.status), marker=L.circleMarker([point.lat,point.lng],{radius:pointIndex===0?8:6,color,fillColor:color,fillOpacity:.92}).addTo(map);marker.on('click',()=>showDetail(route,point));bounds.push([point.lat,point.lng]);}); const last=route.at(-1), motor=L.marker([last.lat,last.lng],{icon:L.divIcon({className:'tracking-motor-wrap',html:'<div class="tracking-motor">&#128100;&#127949;</div>',iconSize:[42,42],iconAnchor:[21,21]})}).addTo(map);motor.on('click',()=>showDetail(route)); });
