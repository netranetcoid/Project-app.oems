...
<div class="card mb-4">
  ...
  <div class="card-body"><div class="row g-3"><div class="col-lg-8"><div id="workTrackingMap" style="height:560px" class="rounded"></div></div><div class="col-lg-4"><div id="routeDetail" class="tracking-detail"><h5 class="mb-2">Detail perjalanan</h5><p class="text-muted mb-0">Peta menampilkan seluruh titik GPS yang diterima. Klik motor atau titik berhenti untuk melihat detail lokasi dan task aktif.</p></div></div></div></div>
</div>
...
<script>
  window.addEventListener('load', function () {
    const mapElement = document.getElementById('workTrackingMap');
    if (!window.L) {
      mapElement.innerHTML = '<div class="alert alert-danger m-3">Library peta gagal dimuat. Hubungi developer dan refresh halaman setelah cache dibersihkan.</div>';
      return;
    }
    try {
  ...
  let showDetail = (route, selectedPoint = null) => { const last=route.at(-1), point=selectedPoint || last, stop=stopInfo(route), isLastPoint=point === last, stopped=Boolean(point.stopSeconds) || (isLastPoint && stop.seconds>=600), stopSeconds=point.stopSeconds || stop.seconds; const activity=point.activity ? point.activity : 'Belum ada task aktif/dilaporkan'; const sessionStatus=point.isActive ? 'Masih aktif â€” posisi terbaru saat ini' : 'Sudah out â€” posisi terakhir yang tercatat'; detail.innerHTML=`<div class="d-flex align-items-center gap-2"><span class="tracking-pulse"></span><div><h5 class="mb-0">${point.name || 'Pegawai'}</h5><small class="text-muted">${point.email || '-'}</small></div></div><div class="tracking-stat"><small class="text-muted d-block">${isLastPoint ? 'Titik terakhir' : (stopped ? 'Titik berhenti' : 'Titik perjalanan')}</small><strong>${point.lat.toFixed(6)}, ${point.lng.toFixed(6)}</strong><small class="text-muted d-block mt-1">Terekam ${point.time} - ${point.mode === 'overtime' ? 'Lembur' : 'Jam kerja'}</small></div><div class="tracking-stat"><small class="text-muted d-block">Status sesi</small><strong>${sessionStatus}</strong></div><div class="tracking-stat"><small class="text-muted d-block">Status lokasi</small><strong>${stopped ? 'Berhenti sekitar '+duration(stopSeconds) : 'Bergerak / belum 10 menit'}</strong><small class="text-muted d-block mt-1">Radius berhenti maksimal 35 m.</small></div><div class="tracking-stat"><small class="text-muted d-block">Kegiatan (task aktif)</small><strong>${activity}</strong><small class="text-muted d-block mt-1">GPS hanya menunjukkan lokasi; kegiatan berasal dari task, bukan tebakan sistem.</small></div><div class="tracking-stat"><small class="text-muted d-block">Validasi sistem</small><strong class="${point.status==='blocked'?'text-danger':point.status==='review'?'text-warning':'text-success'}">${statusLabel(point.status)}</strong></div>`; };
  const motorSvg = (state) => `<svg class="tracking-motor ${state === 'reached' ? '' : (state ? 'is-active' : 'is-out')}" style="color:${state === 'reached' ? '#16a34a' : ''}" viewBox="0 0 104 84" aria-label="Posisi motor"><circle cx="25" cy="62" r="13" fill="#0f172a"/><circle cx="80" cy="62" r="13" fill="#0f172a"/><circle cx="25" cy="62" r="6" fill="#e2e8f0"/><circle cx="80" cy="62" r="6" fill="#e2e8f0"/><path d="M25 58h20l10-23h18l10 23H48" fill="none" stroke="currentColor" stroke-width="8" stroke-linecap="round" stroke-linejoin="round"/><path d="M52 32l11-13h12" fill="none" stroke="currentColor" stroke-width="7" stroke-linecap="round"/><circle cx="58" cy="16" r="8" fill="#f2b705"/><path d="M43 39h22" stroke="#38bdf8" stroke-width="6" stroke-linecap="round"/></svg>`;
  routes.forEach((route)=>{ const visible=route; const hasReview=visible.some((point)=>point.status==='review'); if(visible.length>1){const line=L.polyline(visible.map((point)=>[point.lat,point.lng]),{color:hasReview?'#f59e0b':'#0284c7',weight:hasReview?2:3,opacity:.78,dashArray:hasReview?'6 6':null}).addTo(map);line.bindTooltip(hasReview?'Rute mengandung titik perlu review':'Rute tervalidasi');line.on('click',()=>showDetail(route));} if(route.length){const start=route[0];L.circleMarker([start.lat,start.lng],{radius:5,color:'#16a34a',fillColor:'#16a34a',fillOpacity:1}).addTo(map).bindTooltip('Mulai');} const last=route.at(-1), finalStop=stopInfo(route); stopPoints(route).filter((point)=>point.time!==last.time).forEach((point)=>{const marker=L.marker([point.lat,point.lng],{icon:L.divIcon({className:'tracking-motor-wrap',html:motorSvg('reached'),iconSize:[52,42],iconAnchor:[26,34]})}).addTo(map);marker.bindTooltip('Titik sudah dijangkau');marker.on('click',()=>showDetail(route,point));}); if(last.isActive && finalStop.seconds>=600){L.circleMarker([last.lat,last.lng],{radius:24,color:'#f59e0b',weight:3,fillColor:'#fbbf24',fillOpacity:.28}).addTo(map).bindTooltip('Berhenti minimal 10 menit');} const motor=L.marker([last.lat,last.lng],{icon:L.divIcon({className:'tracking-motor-wrap',html:motorSvg(last.isActive),iconSize:[52,42],iconAnchor:[26,34]})}).addTo(map);motor.bindTooltip(last.isActive ? 'Posisi terbaru (aktif)' : 'Posisi terakhir (sudah out)');motor.on('click',()=>showDetail(route));route.forEach((point)=>bounds.push([point.lat,point.lng])); });
  ...
  const resolveAddress = (point) => {
    const slot = document.querySelector('[data-tracking-address]');
    if (!slot || !point) return;
    slot.textContent = 'Mencari alamat titik iniÃ¢â‚¬Â¦';
    fetch(`https://nominatim.openstreetmap.org/reverse?format=jsonv2&zoom=18&lat=${encodeURIComponent(point.lat)}&lon=${encodeURIComponent(point.lng)}`)
      .then((response) => response.ok ? response.json() : Promise.reject())
      .then((data) => { slot.textContent = data.display_name || 'Alamat tidak tersedia; koordinat audit tetap tersimpan.'; })
      .catch(() => { slot.textContent = 'Alamat belum tersedia; koordinat audit tetap tersimpan.'; });
  };
  const previousShowDetail = showDetail;
  showDetail = (route, selectedPoint = null) => {
    previousShowDetail(route, selectedPoint);
    const point = selectedPoint || route.at(-1);
    detail.insertAdjacentHTML('beforeend', '<div class="tracking-stat"><small class="text-muted d-block">Alamat titik</small><strong data-tracking-address>Memuat alamatÃ¢â‚¬Â¦</strong></div>');
    resolveAddress(point);
  };
  if(bounds.length){map.fitBounds(bounds,{padding:[32,32]});showDetail(routes.at(-1));}
  // Peta HR aktif diperbarui setiap 30 detik tanpa perlu refresh manual.
  if (@json($hasActiveJourneys)) window.setTimeout(() => window.location.reload(), 20000);
  window.setTimeout(() => map.invalidateSize(), 200);
    } catch (error) {
      console.error('OvallHR tracking map failed', error);
      mapElement.innerHTML = '<div class="alert alert-danger m-3">Peta tidak dapat dibuat. Data perjalanan tetap aman; buka Console browser untuk detail.</div>';
    }
  });
</script>
@endsection
