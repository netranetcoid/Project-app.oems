...
<div class="card">
  ...
  <div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th>Pegawai / Site</th><th>Shift</th><th>Masuk / Pulang</th><th>Durasi kerja / Keterlambatan</th><th>GPS & Bukti</th><th>Catatan & audit</th><th>Status</th><th class="text-end">Aksi HR</th></tr></thead><tbody>
    @forelse($records as $record)
      ...
      @endphp
      <tr>
        <td>@if($record->employee)<a class="fw-semibold text-body" href="{{ route('employees.show', ['employee' => $record->employee_id, 'tab' => 'attendance']) }}">{{ $record->employee->name }}</a><div class="small text-muted">{{ $record->employee->employee_no ?? '-' }} Ã‚Â· {{ $record->employee?->branch?->name ?? 'Tanpa site' }}</div><a class="small" href="{{ route('employees.show', ['employee' => $record->employee_id, 'tab' => 'attendance']) }}">Lihat riwayat pegawai</a>@else<strong>Pegawai tidak ditemukan</strong>@endif</td>
        <td>{{ $record->shift?->name ?? 'Tanpa shift' }}<div class="small text-muted">{{ $record->shift?->clock_in_time ? substr($record->shift->clock_in_time,0,5) : '-' }}Ã¢â‚¬â€œ{{ $record->shift?->clock_out_time ? substr($record->shift->clock_out_time,0,5) : '-' }}</div></td>
        <td><div class="fw-semibold"><span class="text-success">Masuk {{ $record->clock_in_at?->copy()->setTimezone($timezone)->format('H:i') ?? '-' }}</span><span class="text-muted mx-1">Ã¢â€ â€™</span><span class="{{ $record->clock_out_at ? 'text-danger' : 'text-muted' }}">Pulang {{ $record->clock_out_at?->copy()->setTimezone($timezone)->format('H:i') ?? 'Belum tercatat' }}</span></div><div class="small text-muted">Waktu lokal perusahaan</div></td>
        <td><strong>{{ $hours }}</strong><div class="small text-muted">Durasi kerja</div><div class="small {{ $record->late_minutes > 0 ? 'text-warning' : 'text-success' }}">Keterlambatan: {{ $record->late_minutes }} menit{{ $record->shift?->clock_in_time ? '' : ' (tanpa shift)' }}</div></td>
