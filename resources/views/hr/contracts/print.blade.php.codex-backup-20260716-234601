...
Kontrak Kerja - {{ $contract->contract_no }}
...
background:url("{{ asset('assets/img/logo_osm_transparent.png') }}") center no-repeat;
...
<img src="{{ asset('assets/img/logo_osm_transparent.png') }}">
...
<b>{{ now()->translatedFormat('d F Y') }}</b>
...
{{ $contract->employee->identity_number ?? '-' }}
...
{!! nl2br(e($contract->employee->address ?? '-')) !!}
...
<b>{{ $contract->position_name }}</b>

pada Divisi

<b>{{ $contract->division_name }}</b>
...
<b>{{ optional($contract->start_date)->translatedFormat('d F Y') }}</b>

@if($templateKey === 'probation')
dengan masa percobaan/evaluasi sampai dengan

<b>{{ optional($contract->probation_end_date)->translatedFormat('d F Y') }}</b>

dan hubungan kerja tidak berakhir otomatis pada tanggal evaluasi.
@elseif($templateKey === 'internship')
sampai dengan <b>{{ optional($contract->end_date)->translatedFormat('d F Y') }}</b>
...
{{ $templateKey === 'internship' ? 'Uang Saku / Imbalan Program' : 'Gaji Pokok' }}
...
Rp {{ number_format($contract->position_allowance ?? 0,0,',','.') }}
...
Rp {{ number_format($contract->transport_allowance ?? 0,0,',','.') }}
...
Rp {{ number_format($contract->meal_allowance ?? 0,0,',','.') }}
...
Rp {{ number_format($contract->fixed_allowance ?? 0,0,',','.') }}
...
<u><b>{{ strtoupper($contract->employee_name) }}</b></u>
